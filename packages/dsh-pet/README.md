...
- **Master switch**: `enabled` owns the `/api/pet/*` routes' lifetime — the host registers them while the switch is on and withdraws them when it is off. Both browser consumers follow that verdict: the floating sprite stops polling, and the settings card issues no registry request. The settings card itself stays registered while the pet is off, because it is where the switch is turned back on.
